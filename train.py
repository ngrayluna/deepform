...
"num_docs" : 80,    # how many docs to actually use
"window_len" : 30,  # size of token sequences to train on (and network size!)
"vocab_size" : 500,
"token_dims" : 8,   # number of features per token, including token hash
"amount_feature": 1, # let the model see the numeric value of the token?
...
"val_split" : 0.2,
"pad_windows" : 1,   # Pad documents (both before start and end of doc.) 

}

seed = 42

run = wandb.init(project="deepform", entity="deepform", name="padding")
config = run.config
...
def load_training_data(config):
    ...
    random.seed(seed)
    slugs = random.sample(slugs, config.num_docs)
    print("Length of slugs in load_training_data after modification = ", len(slugs))
    
    random.seed(seed)
    token_text = random.sample(token_text, config.num_docs)
    random.seed(seed)
    features = random.sample(features, config.num_docs)
    random.seed(seed)
    labels = random.sample(labels, config.num_docs)
